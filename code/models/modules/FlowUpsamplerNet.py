...
class FlowUpsamplerNet(nn.Module):
    def __init__(self, image_shape, hidden_channels, K, L=None,
        ...
        for level in range(1, self.L + 1):
            # 1. Squeeze
            if self.model_name == "SRFlow-DA-S":
                H, W = self.arch_squeeze(H, W, opt['scale'])
    ...
    def arch_FlowStep(self, H, K, LU_decomposed, W, actnorm_scale, affineInCh, flow_coupling, flow_permutation,
        ...
        for k in range(K):
            ...
            if self.model_name == "SRFlow-DA-S":
                if k % 6 == 0:
                    self.arch_split(H, W, 1, self.L, opt, opt_get)
    ...
    def arch_squeeze(self, H, W, factor=2):
        self.C, H, W = self.C * factor**2, H // factor, W // factor
        self.layers.append(flow.SqueezeLayer(factor=factor))
    ...
    def decode(self, rrdbResults, z, eps_std=None, epses=None, logdet=0.0, y_onehot=None):
        ...
        for layer, shape in zip(reversed(self.layers), reversed(self.output_shapes)):
            ...
            if isinstance(layer, Split2d):
                ...
                fl_fea, logdet = layer(fl_fea, logdet=logdet, reverse=True, rrdbResults=rrdbResults[self.levelToName[level]])
            else:
